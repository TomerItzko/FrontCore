...
public class FastSnowParticleEffect
extends AbstractParticleEffect {
    private static final int field_7053 = 8;
    private static final float field_7054 = 0.02f;
        ...
        for (int i = -8; i < 8; ++i) {
            for (int j = -8; j < 8; ++j) {
                for (int k = -8; k < 8; ++k) {
                    ...
                    if (!(random.nextFloat() < 0.02f) || level.getBlockState(blockPos).canOcclude() || !level.canSeeSky(blockPos)) continue;
                    Vec3 vec3 = cameraPos.add((double)i, (double)j, (double)k);
                    ClientUtils.spawnParticle(minecraft, manager, level, (SimpleParticleType)BFParticleTypes.FALLING_SNOW_FAST_PARTICLE.get(), vec3.x, vec3.y, vec3.z, 0.0, 0.0, 0.0);
                }
            }
        }
    }
