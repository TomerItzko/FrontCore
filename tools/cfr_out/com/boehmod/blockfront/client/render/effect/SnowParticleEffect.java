...
public class SnowParticleEffect
extends AbstractParticleEffect {
    private static final int field_324 = 8;
        ...
        for (int i = -8; i < 8; ++i) {
            for (int j = -8; j < 8; ++j) {
                for (int k = -8; k < 8; ++k) {
                    ...
                    if (!((double)random.nextFloat() < 0.01) || level.getBlockState(blockPos).canOcclude() || !level.canSeeSky(blockPos)) continue;
